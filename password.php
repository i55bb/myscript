    ...
    public function load( )
    {
        ...
        if ( $this->isPost( ) && isset( $_POST['id'] ) && isset( $_POST['email'] ) && is_numeric( $_POST['id'] ) )
        {
            ...
            if ( $this->pageState == 3 )
            {
                ...
                $link = WebHelper::getbaseurl( )."password.php?id=".$playerId."&n=".$n."&c=".substr( md5( bin2hex( $playerId ).$name."777" ), 7, 7 );
                $to = $email;
        ...
        else if ( isset( $_GET['id'] ) && is_numeric( $_GET['id'] ) )
        {
            ...
            if ( isset( $_GET['n'] ) && trim( $_GET['n'] ) != "" && isset( $_GET['c'] ) )
            {
                if ( $this->pageState == 1 )
                {
                    ...
                    if ( trim( $_GET['c'] ) == substr( md5( bin2hex( $this->playerId ).$name."777" ), 7, 7 ) )
                    {
                        $newPassword = base64_encode(md5(bin2hex(convert_uuencode(json_encode($this->playerId)))));
